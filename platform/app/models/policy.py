...
class Policy(UUIDPrimaryKeyMixin, TimestampMixin, db.Model):
    ...
    created_by = db.Column(db.Uuid(), db.ForeignKey("users.id"))
...
class AccessGrant(UUIDPrimaryKeyMixin, db.Model):
    ...
    subject_id = db.Column(db.Uuid(), nullable=False)
    server_scope_type = db.Column(db.Text, nullable=False)
    server_scope_id = db.Column(db.Uuid())
    unix_principals = db.Column(db.JSON, nullable=False, default=list)
    policy_id = db.Column(db.Uuid(), db.ForeignKey("policies.id"))
