...
class UserGroup(db.Model):
    ...
    group_id = db.Column(db.Uuid(), db.ForeignKey("groups.id"), primary_key=True)
...
class RoleBinding(UUIDPrimaryKeyMixin, TimestampMixin, db.Model):
    ...
    role_id = db.Column(db.Uuid(), db.ForeignKey("roles.id"), nullable=False)
    scope_type = db.Column(db.Text, nullable=False)
    scope_id = db.Column(db.Uuid())
